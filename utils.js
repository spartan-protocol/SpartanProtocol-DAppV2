...
const Element =
  (typeof window === 'object' && window.Element) || console.log('nope')
...
export function toNumber(value) {
  ...
  if (isObject(value)) {
    ...
    _value = isObject(other) ? `${other}` : other
  }
  if (type !== 'string') {
    return _value === 0 ? _value : +_value
  }
  _value = _value.replace(/^\s+|\s+$/g, '')
  const isBinary = /^0b[01]+$/i.test(_value)
  return isBinary || /^0o[0-7]+$/i.test(_value)
    ? parseInt(_value.slice(2), isBinary ? 2 : 8)
    : /^[-+]0x[0-9a-f]+$/i.test(_value)
    ? NAN
    : +_value
}
