...
import {Alert, CardBody, ModalBody} from "reactstrap";
        ...
        <>
            ...
            >
                ...
                <Modal.Body className="center-text">
                    {wallet.status === 'error' &&
                    <Alert color="warning" >
                        <span> Wallet connection failed! Check the network is set to BSC MainNet!
                        Have you selected the correct wallet type?</span>
                    </Alert>
                    ...
                    {wallet.status === 'connected' ? (
                        ...
                        <div>
                            {walletTypes.map(x => (
                                <div key={x.id}>
                                    <button size="lg" color="success" type="button"
                                            className="btn btn-warning btn-block"
                                            onClick={() => connectWallet(props, x)}>
                                        <Col>
                                            <div className="float-left mt-2 ">{x.title}</div>
                                            <div className="float-right"><Image src={x.icon}
                                                                                className='px-1 wallet-icons'/></div>
