...
const Common = () => {
	...
	const getRoutes = (tempRoutes) =>
		tempRoutes.map((prop) => {
			...
			// if (prop.layout === '/dapp') {
			return <Route path={prop.path} component={prop.component} key={prop.path + prop.name} />
			// }
			// return null
		})
	...
	return (
		...
		>
			...
			<div className="navbar-minimize-fixed" style={{ opacity: 0 }}>
				<button type="button" className="minimize-sidebar btn btn-link btn-just-icon" onClick={handleMiniClick}>
			...
			<div className="main-panel" ref={mainPanelRef} data={activeColor}>
				...
				</Switch>
				{
					...
					location.pathname.indexOf('full-screen-map') !== -1 ? null : <Footer />
				}
			</div>
		</div>
	)
